...
%   Example of use:  InfoTable = f_InfoTable(Barcodes{b}, 'S:\OperaQEHS\OperaDB\Jonathan Arias\JA20161203_AllBasal_AutoMito_LC3\Settings\basal_conditions.lay');
    ...
    TimePoint = regexp(files, '.*\\(.*)\\.*', 'tokens'); 
    RowColField = regexp(files, '.*\\(.*).flex', 'tokens');
    TimePoint = cellfun(@(x) str2double(x{:}{:}(6:7)), TimePoint, 'UniformOutput', false);
    ...
    files = [cell2table(files), cell2table(Row), cell2table(Column), cell2table(field), cell2table(TimePoint)];
